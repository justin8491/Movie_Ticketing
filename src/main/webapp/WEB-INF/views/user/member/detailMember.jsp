...
<body>
	...
	<section class="content">
				...
				<form class="float_form row">
					...
					<div class="form-group row float">
						<div class="box">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="mem_password"
									value="${member.mem_password}" placeholder="비밀번호" readonly />
							</div>
						</div>
					</div>
					<div class="form-group row float">
						<div class="box">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="mem_name"
									value="${member.mem_name}" placeholder="이름" readonly />
							</div>
						</div>
					</div>
					<div class="form-group row float">
						<div class="box">
							<label for="mem_ssn" class="col-sm-2 col-form-label">주민번호</label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="mem_ssn"
									value="${member.mem_ssn}" placeholder="주민등록번호" readonly />
							</div>
						</div>
					</div>
					<div class="form-group row float">
						<div class="box">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="mem_phoneNumber"
									value="${member.mem_phoneNumber}" placeholder="전화번호" readonly />
							</div>
						</div>
					</div>
						...
						<div class="box">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="mem_address"
									value="${member.mem_address}" placeholder="주소" readonly />
							</div>
						</div>
					</div>
					...
					<div class="form-group row float">
						<div class="box">
							<label for="mem_accumulatedAmount"
								class="col-sm-2 col-form-label"></label>
							<div class="col-sm-10">
								<input type="hidden" class="form-control"
									id="mem_accumulatedAmount"
									value="${member.mem_accumulatedAmount}" placeholder="누적금액"
									readonly />
							</div>
						</div>
					</div>
							...
							<div class="offset-sm-2">
                              ...
                              > -->
								<a class="btn btn-warning"
									href="${contextPath}/user/updateMemberForm">개인정보 변경</a>
							</div>
						</div>
					</div>
				</form>
		...
		<div class="container mt-5 aabbcc">
			<h2>예약 내역</h2>
					...
					<c:forEach var="reservation" items="${dataList[0].reservationList}">
						<tr>
							...
							<td style="border: 1px solid #ddd; padding: 8px;">
								<button
									onclick="cancel(${reservation.res_no}, ${reservation.pay_no})"
									style="padding: 5px 10px; border: none; background-color: #f44336; color: white; cursor: pointer;">예매취소</button>
							</td>
						</tr>
					</c:forEach>
				</table>
				...
				<div class="reviewInfo border heihgt">
					<c:forEach var="item" items="${reviewList}">
						<div class="review-item" id="review-${item.rev_no}">
							...
							<!-- 영화 제목 표시 할 방법 찾기 -->
							<span>${item.mo_name}</span> <span class="review-content"
								id="review-content-${review.rev_no}">${item.rev_content}</span>
							<div class="star-rating" id="review-rating-${review.rev_no}"
								title="Rated ${item.rev_rating}.00 out of 5">
								<span style="width:${item.rev_rating * 20}%"> <strong
									class="rating">${item.rev_rating}.00</strong> out of 5
								</span>
							</div>
					...
					<table>
						...
						<c:forEach var="inquiry" items="${myBoard}">
							<c:if test="${member.mem_id == inquiry.bo_writerId}">
								<tr>
									<c:choose>
											<c:when test="${inquiry.cat_no == 3}">
												<td><a href="/user/support/selectFreeBoard?bo_no=${inquiry.bo_no}">${inquiry.bo_no}</a></td>
												<td><a href="/user/support/selectFreeBoard?bo_no=${inquiry.bo_no}">${inquiry.bo_title}</a></td>
											</c:when>
											<c:when test="${inquiry.cat_no == 4}">
												<td><a href="/user/support/selectInquiry?bo_no=${inquiry.bo_no}">${inquiry.bo_no}</a></td>
												<td><a href="/user/support/selectInquiry?bo_no=${inquiry.bo_no}">${inquiry.bo_title}</a></td>
											</c:when>
										</c:choose>
