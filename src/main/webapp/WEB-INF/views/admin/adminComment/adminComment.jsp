                            ...
                            $.ajax({
                               ...
                               error : function(){
								...
								<input type="hidden" id="mem_id" name="mem_id"
									value="${member.mem_id}" /> <input type="hidden" id="bo_no"
									name="bo_no" value="${freeBoard.bo_no}" />
							</form>
						</div>
						<div class="container">
							<div class="row">
								...
								<c:forEach var="item" items="${adminCommentList}">
									<div class="comment-item" id="comment-${item.com_no}">
										<span>${item.mem_id}</span> <span class="comment-content"
											id="comment-content-${item.com_no}">${item.com_content}</span>
										<span>${item.com_createdAt}</span>
										<button onclick="adminCommentDelete(${item.com_no})">삭제</button>
										<form class="edit-form" id="edit-form-${item.com_no}"
											style="display: none;" onsubmit="return false;">
											<input id="mem_id" name="mem_id" value="${member.mem_id}"
												readOnly />
										</form>
									</div>
									<br>
